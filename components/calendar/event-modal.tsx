...
import { Calendar, MapPin, Cloud, Video, Trash2 } from "lucide-react";
...
import { toast } from "sonner"; // make sure you installed `sonner`
  ...
  const onSubmit = (data: EventFormData) => {
    onSave({
      ...
      attendees: data.attendees.map((a) => ({
        ...a,
        status: (a as any).status ?? "pending", // ensure `status` exists
      })),
    });
  };
          ...
          {showAIAssistant && (
            <AISchedulingAssistant
              ...
              onSuggestionSelect={(suggestion) => {
                setValue(
                  "start",
                  format(suggestion.suggestedTime, "yyyy-MM-dd'T'HH:mm")
                );
                setValue(
                  "end",
                  format(
                    new Date(
                      suggestion.suggestedTime.getTime() + 60 * 60 * 1000
                    ),
                    "yyyy-MM-dd'T'HH:mm"
                  )
                );
